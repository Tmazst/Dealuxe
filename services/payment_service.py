...
class MojaPOSService:
    ...
    def initiate_wallet_topup(self, user_id, amount, phone_number):
        """Initiate a player loading funds into their wallet."""
        external_txn_id = f"topup_{user_id}_{uuid.uuid4().hex[:8]}"

        if self._mock_mode:
            return {
                'success': True,
                'external_transaction_id': f"mock_topup_{user_id}",
                'payment_url': None,
                'mock': True,
            }
